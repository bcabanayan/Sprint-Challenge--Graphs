    ...
    while move_queue:
        for direction in move_queue:
            ...
            # if the new room value is not in the traversal graph...
                ...
                # UNEXPLORED ROOM IS A DEAD END!!!!
                elif len(unexplored) == 0:
                    # ****** maybe if exits == 1, then update this so that they back away from the dead end before executing all of the other code, by counting number of exits in line 131 above!!!!**********
                    # exits = player.currentRoom.getExits()
                    # if len(exits) == 1:
                    #     if direction == 'n':
                    #         move_queue.append('s')
                    ...
                    while q.size() > 0:
                        ...
                        if vertex not in visited:
                            if '?' in q.queue[0].values():
                                ...
                                return print('path is ', path, 'rooms are ', vertex)                        
            ...
            # else, if the current room HAS been visited, explore the unvisited rooms
                ...
                if len(unexplored) == 1:
                    ...
                    traversalPath.append(unexplored[rand_index])
                    move_queue.append(unexplored[rand_index])
                # ROOM THAT HAS BEEN VISITED HAS FULLY EXPLORED EXITS!!!!!!!!!
                # elif len(unexplored) == 0:
                # execute BFS here
                print(traversalPath)
